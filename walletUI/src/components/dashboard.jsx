...
const Dashboard = () => {
  ...
  const makeTransfer = async (amount, destination) => {
    const response = await axios.get(
      `http://localhost:5000/api/transfer?amount=${amount}&wallet=${
        user ? user.wallets[wallet].id : ""
        ...
        <div className="left_1">
          {user ? (
            <div>
              ...
              {!token?.length && (
                <div className="empty">you have no tokens yet</div>
              )}
              {token?.length && (
                  ...
                  {token.map((tk) => {
                    return (
                      <div className="token">
                        <div className="info">
                          <img
                            src={tk.token.symbol === "USDC" ? usdc : sol}
                            alt="d"
                          />
