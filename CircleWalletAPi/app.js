...
passport.use(new GoogleStrategy({
    ...
    const wallet_set = await createWalletSet(profile.displayName.split(' ').join('__'))
...
app.get('/api/transfer', async(req, res) => {
    ...
    if (!req.user) return res.send({ error: true, message: 'Invalid transaction1' })

    let amount = parseInt(req.query.amount || 0)
    if (amount == 0) return res.send({ error: true, message: 'Invalid transaction2' })
    ...
    if (tokens.length === 0) return res.send({ error: true, message: 'Invalid transaction4' })
